...
Server.prototype = {
  ...
  log: function(str) {
    if (this.config.debug) winston.info(str);
    if (this.operchannel && this.channels && this.service)
      this.channels.message(this.service, this.operchannel, str);
  },
  ...
  channelForbidden: function(target) {
    if (this.config.forbidden) {
      for (mask in this.config.forbidden) {
        if (target.match(mask)) {
          if (this.config.forbidden[mask]['reason']) {
            return this.config.forbidden[mask]['reason'];
          } else {
            return true;
          }
        }
  ...
  parse: function(data) {
    ...
    if (parts.length > 0) {
      if (args[0] === 'BROADCAST') {
        return {
          command: 'BROADCAST',
          args: [data.trim().substring(args[0].length + 1)]
        }
      }
      args.push(parts[1]);
    }
  ...
  createDefaultChannels: function() {
    var self = this;
    if (this.config.operchannel) {
      var channelName = this.config.operchannel;
      if (!channelName.match(/^[#&]\w+/)) { 
        channelName = "#" + channelName;
      }
      self.operchannel = self.channels.registered[self.normalizeName(channelName)] = new Channel(channelName, self);
      self.operchannel.topic = "Internal";
    }
  ...
  startTimeoutHandler: function() {
    ...
    this.timeoutHandler = setInterval(function() {
      self.users.forEach(function(user) {
        if (user.hasTimedOut()) {
          self.log('Disconnect due to timeout:', user.mask);
  ...
  registerService: function() {
    ...
    if (server.config.service) {
      ...
      service.nick = server.config.service.nick,
      service.username = server.config.service.nick,
      service.realname = server.config.service.nick,
      service.remoteAddress = server.config.service.hostname;
      service.register();

      setTimeout(function() {
        server.channels.join(service, server.config.operchannel);
        for (channel in server.config.channels) {
          var channelName = channel;
          if (!channelName.match(/^[#&]\w+/)) { 
            channelName = "#" + channelName;
          }
          server.channels.join(service, channelName);
        }
      }, 5000);
  ...
  start: function(callback) {
    ...
    server.log('Server listening on port: ' + this.config.port);
  ...
  data: function(client, line) {
    ...
    if (cmd === 'CAP') {
      this.log(client.object.mask.substring(1) + ' [' + this.name + ' - Connecting]');
    } else if (cmd.match(/(join|part|kick|nick|oper)/i)) {
      if (cmd === 'OPER') line = 'OPER ' + line.split(' ')[1] + ' *********';
      this.log(client.object.mask.substring(1) + ' [' + this.name + ' - ' + line + ']');
    }
  }
};
