...
Commands.prototype = {
  HOST: function(user, host) {
    user.hostname = host;
    user.remoteAddress = host;
    user.hostLookup();
  },
  ...
  PASS: function(user, password) {
    ...
    // e.g. bitcoin address holder verification
    // password = Bitcoinaddress + : + Signed string for String of Bitcoinaddress
    ircd.compareHash(password, self.config.serverPassword, function(err, res) {
      if (res) {
        ...
        if (cryptocoin.isValidAddress(password)) {
          user.coinaddr = password;
        } else {
  ...
  JOIN: function(user, channelNames, key) {
    ...
    channelNames.split(',').forEach(function(args) {
      ...
      } else if (server.channelForbidden(channelName)) {
        var reason = server.channelForbidden(channelName);
        console.log(reason);
        if (reason !== false) {
          user.send(server.host, irc.errors.banned, ':This channelname is not allowed (' + reason + ')');
        } else {
          user.send(server.host, irc.errors.banned, ':This channelname is not allowed');
        }
      } else {
        if (channelName == server.config.operchannel && !user.isOper) {
          return user.send(server.host, irc.errors.needMoreParams, user.nick, ':You need to be IRC Operator for this channel');
        } else {
  ...
  KICK: function(user, channels, users, kickMessage) {
    ...
    if (userNames.length !== channelMasks.length) {
      ...
      channelMasks.forEach(function(channelMask, i) {
        ...
        } else if (!user.isOp(channel) || targetUser.isOper) {
  ...
  MODE: function(user, target, modes, arg) {
    ...
    // d - delayed join channel flag                               [done]
      ...
      if (!channel) {
        ...
        channel.users.forEach(function(channelUser) {
          if (channelUser.isInvisible
            ...
            // H is here, G is gone, * is IRC operator, + is voice, @ is chanop
            user.send(server.host,
                      ...
                      channelUser.nick,
                      channelUser.channelPrivilege(channel),
                      ':0',
    ...
    if (target) {
      var channels = target.channels.map(function(channel) {
        ...
        if (target.isOp(channel)) {
          return '@' + channel.name;
        } else if (target.isVoiced(channel)) {
          return '+' + channel.name;
        } else {
          return channel.name;
        }
      ...
      if (target.coinaddr) {
        user.send(this.server.host, irc.reply.whoIsUser, user.nick, target.nick,
                  'address', 'bitcoin', '*', ':' + target.coinaddr);
      }
  ...
  // TODO: RFC
  KILL: function(user, target, message) {
    if (user.isOper) {
      var targetUser = this.server.users.find(target);
      if (targetUser) {
        targetUser.quit('KILLED (' + message + ')');
        this.server.history.add(targetUser);
        delete targetUser;
      }
    }
  },
  ...
  BROADCAST: function(user, message) {
    if (user.isOper) {
      var _this = this, service = this.server.service;
      this.server.users.registered.forEach(function(target) {
        service.message(target.nick, "**** Broadcast by " + user.nick + " ****");
        service.message(target.nick, message);
        service.message(target.nick, "**** End of broadcast ****");
      });
    } else {
      user.send(this.server.host, irc.errors.noPrivileges, user.nick, ":Permission denied");
    }
  }
};
