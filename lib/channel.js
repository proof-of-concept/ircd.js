...
Channel.prototype = {
  ...
  get isDelayedJoin() {
    return this._modes.indexOf('d') > -1;
  },

  get isAutoDelay() {
    return this._modes.indexOf('a') > -1;
  },
  ...
  addMode: {
    ...
    v: function(user, arg) {
      if (user.isHop(this) || user.isOper) {
        var targetUser = this.findUserNamed(arg);
    ...
    d: function(user, arg) {
      this.opModeAdd('d', user, arg);
    },

    a: function(user, arg) {
      this.opModeAdd('a', user, arg);
    },
  ...
  removeMode: {
    ...
    d: function(user, arg) {
      if (this.delayedJoins.length > 0) {
        var that = this;
        this.delayedJoins.forEach(function(duser, i) {
          that.users.forEach(function(channelUser) {
            if (channelUser !== duser) {
              channelUser.send(duser.mask, 'JOIN', that.name);
            }
          });
          delete that.delayedJoins[i];
        });
      }
      this.opModeRemove('d', user, arg);
      this.opModeRemove('a', user, arg);
    },
