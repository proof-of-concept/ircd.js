...
ChannelDatabase.prototype = {
  message: function(user, channel, message) {
    ...
    if (channel.isDelayedJoin && channel.delayedJoins.indexOf(user) !== -1) {
      channel.users.forEach(function(channelUser) { 
        if (channelUser !== user)
          channelUser.send(user.mask, 'JOIN', channel.name);
      });
      channel.delayedJoins.splice(channel.delayedJoins.indexOf(user), 1);
    }
    channel.users.forEach(function(channelUser) {
      if (channelUser !== user) {
  ...
  join: function(user, channelName, key) {
    ...
    if (!channel) {
      if (!channelName.match(/^#[\w.-_]*$/) && channelName.length <= 201) {
        // todo rfc
        user.send(this.server.host, irc.errors.banned, user.nick, channelName, ':Cannot join channel (wrong channel format)');
        return;
      }
      this.server.log('Channel created: ' + channelName);
      channel = this.registered[this.server.normalizeName(channelName)] = new Channel(channelName, this.server);
    } else {
      channel = this.registered[this.server.normalizeName(channelName)];
    }
    ...
    if (channel.users.length === 1 && channel.users[0] === user) {
      user.op(channel);
    }

    if (channel.isDelayedJoin) {
      channel.delayedJoins.push(user);
      user.send(user.mask, 'JOIN', channel.name);
    } else {
      channel.users.forEach(function(channelUser) { 
        channelUser.send(user.mask, 'JOIN', channel.name);
      });

      if (channel.isAutoDelay) {
        if (channel.users.length === 19) {
          var _server = this.server;
          setTimeout(function() {
            channel.addMode["d"].apply(channel, [_server.service]);
            user.send(_server.service.mask, 'MODE', channel.name, '+d');
          }, 1000);
        }
      }
    }
  ...
  remove: function(channel) {
    this.server.log('Channel destroyed: ' + channel.name);
