...
User.prototype = {
  ...
  channelNick: function(channel) {
    if (this.isOp(channel)) return '@' + this.nick;
    else if (this.isHop(channel)) return '%' + this.nick;
    else if (this.isVoiced(channel)) return '+' + this.nick;
    else return this.nick;
  },

  channelPrivilege: function(channel) {
    var userPrivilege = 'H';
    if (this.isOp(channel)) userPrivilege += '@';
    else if (this.isHop(channel)) userPrivilege += '%';
    else if (this.isVoiced(channel)) userPrivilege += '+';
    if (this.isOper) userPrivilege += '*';
  ...
  op: function(channel) {
    if (!this.channelModes[channel] || !this.channelModes[channel].match(/o/))
  ...
  hop: function(channel) {
    if (!this.channelModes[channel] || !this.channelModes[channel].match(/h/))
  ...
  voice: function(channel) {
    if (!this.channelModes[channel] || !this.channelModes[channel].match(/v/))
  ...
  hasTimedOut: function() {
    // add 10 more seconds until timeout
    return this.lastPing && ((Date.now() - this.lastPing) / 1000 > (this.config.pingTimeout || this.config.idleTimeout) + 10000);
  },
